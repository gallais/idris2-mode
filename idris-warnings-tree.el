...
(defun idris-show-source-location (filename lineno col &optional is-same-window)
  (idris-goto-source-location filename lineno col is-same-window))

(defun idris-get-fullpath-from-idris-file (filename)
  "Returns the full filepath of a filename receives from the inferior idris process"
  (concat (file-name-as-directory idris-process-current-working-directory) filename)
  )

(defun idris-goto-location (fullpath)
...
idris-goto-source-location"
  (let ((buf (idris-goto-location fullpath)))
    (set-buffer buf)
...
(defun idris-goto-source-location (filename lineno col is-same-window)
...
a preview and offer to widen."
  (idris-goto-source-location-full (idris-get-fullpath-from-idris-file filename) lineno col is-same-window))

;;;;;; Tree Widget
