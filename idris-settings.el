...
(defcustom idris-source-locations '()
  "A hack to specify location(s) of idris source. This is used for finding definitions of idris symbols that are not in the current source path for use with idris-jump-to-def, etc. (It's a hack because it should be related to the libraries included in the project being compiled, but it is not. If you switch projects, you will have to update this)"
  :type '(repeat file)
  :group 'idris)
